...
const RejectedRides = () => {
  ...
  const rejectedTrips = trips.filter(
    (trip) => trip.requestStatus === "rejected"
  );

  useEffect(() => {
    console.log("Rejected Trips", rejectedTrips);
    setRidesData(rejectedTrips);
  }, []);
  ...
  return (
    <AppScreen>
      <AppHeader title="Rejected Rides" onPress={() => navigation.goBack()} />
      <View style={{ flex: 1 }}>
        {ridesData.length >= 1 && (
          <View style={styles.mainContainer}>
            ...
            <ScrollView>
              <Text style={styles.text}>Rejected Rides</Text>
              <FlatList
                data={ridesData}
        ...
        {ridesData.length === 0 && (
          <View style={styles.mainContainer}>
            <Text style={styles.text}>No Rejected Rides</Text>
          </View>
        )}
      </View>
    </AppScreen>
